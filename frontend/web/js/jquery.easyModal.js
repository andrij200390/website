...
$(function() {
    $('.modal').each(function () {
        var modal_width = $(this).data('modal-width');
        var input_width = $(this).data('input-width');
        if (modal_width) {
            $(this).find('.modal__content').css({'width':modal_width+'px'});
        }
        if (input_width) {
            $(this).find('input').css({'width':input_width+'px'});
        }
    });
	$('.modal').easyModal({
		overlayOpacity: 0.8,
